...
// If you're using v-show, the directive already detects the click outside before the element is
// visible.
...
function execHandler({ el, event, handler, middleware }) {
  if (middleware(event)) {
    handler(event)
  }
}
...
function onFauxIframeClick({ el, event, handler, middleware }) {
  ...
  setTimeout(() => {
    ...
    if (activeElement && activeElement.tagName === 'IFRAME' && !el.contains(activeElement)) {
      execHandler({ el, event, handler, middleware })
    }
  }, 0)
}
